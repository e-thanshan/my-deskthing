...
1. Push this repo to `https://github.com/e-thanshan/my-deskthing`.
2. In **Settings > Pages**, set the source to **Deploy from a branch**, branch `gh-pages`, folder `/ (root)`.

The catalog is published to `https://e-thanshan.github.io/my-deskthing/catalog.v1.json`, which can be submitted to <bridgething.com/apps>
...
With more than one app in `apps/` your commands must specify which one: `bun run dev dash`.

Screenshot for the store listing:

```sh
bun run shot dash            # grabs what is on the screen
bun run shot dash --replace  # overwrite
```
...
```sh
bun run bump dash patch -m "Fix the wind direction arrow"
git commit -am "dash: fix the wind direction arrow" && git push
```
